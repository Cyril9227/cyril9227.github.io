...
Main tool to solve this problem is the [Depth First Search (DFS)](https://en.wikipedia.org/wiki/Depth-first_search) algorithm. <br><i>Courtesy of Claude:</i>

<ul>
  <li>You start in one room. This is your starting point.</li>
  ...
  <li>Sometimes you'll end up in a room with no new doors (all doors lead to rooms you've already visited). When this happens, you go back to the previous room.</li>
...
<ul>
<li>1) `a` -> dfs(`c`) -> dfs(`d`) (forward) -> append(`d`) -> append(`c`) (backtrack) -> append(`a`) (finally append current node)</li>
<li>2) `b` -> dfs(`c`) (already visited so we stop here) -> append(`b`) (append current node)</li>
</ul>
...
Why is it useful ? If we have a sorted computation graph, we can easily compute the gradients of each node with respect to the final output (chain-rule). Something like that : 
...
TODO better explain the difference topsort and dfs

...
OK, we can move on to something a bit more complex. Hidden Markov Chains are a generalization of Markov chains where the state is not directly observable. Instead, we observe a sequence of outputs generated by the state. The goal is to infer the sequence of states given the sequence of outputs.

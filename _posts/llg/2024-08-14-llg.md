...
2)<br><br>
3) Soit $z \in \mathbb{C}$, il s'écrit ainsi $z = a + ib$, $a,b$ réels. On a donc $$x + zy = x + (a + ib)y = (x + ay) + i(by)$$. Si  $z \in \mathbb{R}$ c'est-à-dire $b = 0$ alors $$x + zy = 0 \Leftrightarrow x + ay = 0$$, on a par ex $a = \frac{-x}{y}$ solution avec $y$ non nul. 
Réciproquement si $z$ n'est pas réel, alors $$x + zy = 0 \Leftrightarrow (x + ay) + i(by) = 0$$, en identifiant partie réelle et imaginaire on a bien $x = y = 0$. Rédaction un peu degueue mais on voit bien que graphiquement si on a un nombre complexe qui n'est pas sur la droite des réels, la seule transformation affine qui le ramènerait sur la droite réels est $z \times 0 + 0$..

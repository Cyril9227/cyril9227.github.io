...
## Exercice 389 - 390 - 393 (Racines de l'unité)
...
c) Calculer le produit des éléments de $U_{n}$ <br>
...
c) Le produit est donné par $\prod_{k=0}^{n-1} e^{\frac{2i\pi k}{n}} = e^{\sum_{k=0}^{n-1} \frac{2i\pi k}{n}} = (e^{i \pi})^{n - 1} = (-1)^{n-1}$<br>

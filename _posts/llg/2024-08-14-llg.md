...
Le cas $n = 1$ se vérifie facilement, on considère $n > 1$ et $l$, $p$ $\in {0, ..., n-1}$, on a :
...
On injecte dans la seconde formule, on a :

$$\frac{1}{n} \sum_{l=0}^{n-1}Z_{l} e{^\frac{2 i \pi l p}{n}} = \frac{1}{n} \sum_{l=0}^{n-1} \sum_{j=0}^{n-1}z_{j}e{^\frac{2 i \pi l (p-j)}{n}}$$

On échange les sommes (on peut le faire car les sommes sont finies) :

$$\frac{1}{n} \sum_{j=0}^{n-1}z_{j} \sum_{l=0}^{n-1} (e{^\frac{2 i \pi l}{n}})^{(p-j)}$$

Si on prends un peu de recul, on voit que l'on additionne les $z_{j}$ pondérés par des sommes de racines de l'unité à la puissance $p-j$. On peut utiliser l'exercice precedent pour calculer ces sommes, on a quelques cas à traiter :
...
- Faudrait que je relise un peu sur la DFT et la FFT, c'est loin tout ça
</details>

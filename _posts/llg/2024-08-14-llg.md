...
3. [Exercice 389 - 390](#exercice-389---390-racines-de-lunité)
...
En fait si n divise p, par exemple $p = 6$ et $n = 3$ (ici $U_{3} = \{1, j, j^2}$), si on considère par exemple $j$, on a $j^{6} = (j^{3})^{2}$ et par definition de $j$, $j^{3} = 1$... Bref. <br>
